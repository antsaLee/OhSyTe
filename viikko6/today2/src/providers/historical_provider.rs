...
mod tests {
    ...
    fn provider_has_name() {
        ...
        assert_eq!(p.name(), "HistoricalProvider");
    }

    #[test]
    fn provider_returns_expected_events() {
        let p = HistoricalProvider::new();
        let mut events = vec![];
        p.get_events(&mut events);

        assert_eq!(events.len(), 22);
        assert_eq!(events[0].description, "JDK 1.0 released");
    }
}